...
||漢字の使い方|一般、事例の理解？|17/06/05|
|頂きます。　→　いただきます。|漢字の使い方|「頂」は何かをもらう、頂戴する際に使用。　献上語、丁寧語で使用する際は当てはまらないのでひらがな表記するするのが一般的。|17/06/05|
|画像が左より|レイアウト|画像を中心に寄せ|17/06/07|
|お知らせ画像のテキストの余白|レイアウト|～行目だけ文字間を広くする|17/06/07|
|fmailの全置換のし忘れ|チェック不足|チェック厳重|17/06/08|
|pタグとstrong違い（strongタグは推奨されていない）|認識不足|strongのかわりにpタグ|17/06/08|
|YouTubeの関連動画をオフ|認識不足|YouTube動画貼り付け時は関連動画をオフ|17/06/08|
|YouTubeのレスポンシブ|認識不足|YouTube動画貼り付け時　→　親のセレクタ iframeにwidth: 100%;をかける|17/06/08|
|test領域の報告|認識不足|test領域は作業前に報告|17/06/08|
|カレンダー7月31日がなかった|ミス|31日がない日は西向く侍（2,4,6,9,11）で覚える|17/06/08|
|リスト画像追加について|認識|基本新しいものは初めに|17/06/09|
|本のサブタイトル|認識|既存のコンテンツと比較|17/06/09|
|サーバーのbk、レコードの確認|認識|サーバーに同日のbkがあった場合は前後のレコードを必ず確認、相談|17/06/09|
...
予約サイトに飛ぶバナー等に設置
GAの情報はファイルメーカー
onclickタグニGAのじょうほうを当て込む
GAで対象サイトを検索

管理　→　目標　コンバージョンの項目（目標は消せない。上限は20個）